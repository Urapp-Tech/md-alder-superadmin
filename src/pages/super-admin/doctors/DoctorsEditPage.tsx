...
const DoctorsEditPage = () => {
  ...
  useEffect(() => {
    if (state?.addDateTime?.length) {
      const formattedSchedule = state?.addDateTime?.map((item: any) => ({
        day: item.day,
        openTime: dayjs(item.openTime),
        closeTime: dayjs(item.closeTime),
      }));
      dispatch(setWordDays(formattedSchedule));
  ...
  return (
    <>
      ...
      <TopBar title="Update Doctor Details" isNestedRoute />
                      ...
                      >
                        {imagePreview && (
                          <img
                            src={imagePreview}
                            alt="Preview"
                            className="max-h-[300px] w-full max-w-[300px] rounded"
                          />
                        )}
